...
const CSV_COLUMNS: Array<[string, (record: TrademarkRecord) => string]> = [
  ...
  ["CASE NUMBER", (r) => r.caseNumber], ["CLIENT NAME", (r) => r.clientName],
...
export function DatabasePage() {
  ...
  useEffect(() => {
    ...
    if (tmForm && TM_FORMS.includes(tmForm as TmFormKey)) {
      setFilters((current) => ({ ...current, tmForm: tmForm as TmFormKey }));
      setShowFilters(true);
    }
  }, [location]);
    ...
    <AppShell>
      <div className="flex flex-col h-full bg-white">
        <div className="shrink-0 flex flex-wrap items-center gap-3 px-4 sm:px-6 py-3 bg-[#E8DFC7] border-b-2 border-[#0C0C0C]">
          ...
          <button onClick={handleExport} disabled={exporting || total === 0} className="flex items-center gap-2 px-3 h-10 bg-white border-2 border-[#6C1C1F] text-[#6C1C1F] font-mono font-bold text-xs uppercase tracking-wider disabled:opacity-40"><Download className="w-4 h-4" /> {exporting ? "EXPORTING…" : "CSV"}</button>
        ...
        <div className="flex-1 overflow-auto bg-white">
          <table className="w-full text-left font-mono text-xs whitespace-nowrap border-collapse">
            <thead className="bg-[#0C0C0C] text-[#F0E8D0] sticky top-0 z-10"><tr>{["DATE", "TYPE", "CLIENT CODE", "CASE NO", "APPLICATION", "TM/CPR", "CLASS", "STATUS", "SUB-STATUS", "AGENT", "CITY", "TM FORMS"].map((heading) => <th key={heading} className="px-3 py-3 border-r border-[#333] font-bold tracking-wider text-[10px] last:border-r-0">{heading}</th>)}</tr></thead>
            <tbody>
              {isLoading ? <tr><td colSpan={12} className="px-6 py-12 text-center font-bold text-[#6d6658] animate-pulse">LOADING OPTIMIZED RECORD PAGE…</td></tr>
              : error ? <tr><td colSpan={12} className="px-6 py-12 text-center font-bold text-[#CC0000]">FAILED TO LOAD RECORDS. PLEASE REFRESH.</td></tr>
              : records.length === 0 ? <tr><td colSpan={12} className="px-6 py-12 text-center font-bold text-[#6d6658]">NO RECORDS MATCH THE CURRENT SEARCH OR FILTERS.</td></tr>
              : records.map((record, index) => {
                ...
                return <tr key={record.id} onClick={() => navigate(`/record/${record.id}`)} className={`cursor-pointer border-b border-[#0C0C0C]/10 hover:bg-[#E5D8C8] ${index % 2 === 0 ? "bg-[#FFF9F0]" : "bg-white"}`}>
                  <td className="px-3 py-2 border-r border-[#0C0C0C]/10 text-[#6d6658]">{formatDateShort(record.date)}</td><td className="px-3 py-2 border-r border-[#0C0C0C]/10 font-bold text-[#6C1C1F]">{record.type}</td>
                  <td className="px-3 py-2 border-r border-[#0C0C0C]/10 font-bold">{record.clientCode}</td><td className="px-3 py-2 border-r border-[#0C0C0C]/10 font-bold text-[#0A6B52]">{record.caseNumber}</td>
                  <td className="px-3 py-2 border-r border-[#0C0C0C]/10 max-w-[220px] truncate">{record.appName}</td><td className="px-3 py-2 border-r border-[#0C0C0C]/10 font-bold">{record.tmCprNo}</td>
                  <td className="px-3 py-2 border-r border-[#0C0C0C]/10">{record.appClass}</td><td className="px-3 py-2 border-r border-[#0C0C0C]/10"><span className={`inline-block px-1.5 py-0.5 text-[9px] font-bold border border-[#0C0C0C]/20 ${STAGE_BADGE[record.stage] ?? "bg-[#E8DFC7]"}`}>{record.stage}</span></td>
                  <td className="px-3 py-2 border-r border-[#0C0C0C]/10 text-[#6d6658] max-w-[150px] truncate">{record.subStage}</td><td className="px-3 py-2 border-r border-[#0C0C0C]/10 max-w-[120px] truncate">{record.agent}</td>
                  <td className="px-3 py-2 border-r border-[#0C0C0C]/10">{record.city}</td><td className="px-3 py-2"><div className="flex gap-1">{activeForms.length ? activeForms.map((form) => <span key={form} className="px-1.5 py-0.5 bg-[#B0740E]/15 border border-[#B0740E] text-[#6C1C1F] text-[9px] font-bold">{form}</span>) : <span className="text-[#9d9488]">—</span>}</div></td>
                </tr>;
              })}
            </tbody>
          </table>
        ...
        <div className="shrink-0 flex items-center justify-between px-4 sm:px-6 py-3 bg-[#E8DFC7] border-t-2 border-[#0C0C0C]">
          <span className="font-mono text-[10px] text-[#6d6658] font-bold uppercase tracking-widest">PAGE {page} OF {totalPages} · {total} RECORDS · TYPE / CLIENT / CASE ORDER</span>
