...
export interface TrademarkRecord {
  ...
  notes: string;
  updatedAt: string;
  /** Optimistic concurrency token from trademarks.version */
  version?: number;
  image: string;
...
/**
 * Update a trademark.
 * Pass expectedVersion (from TrademarkRecord.version) to enable optimistic locking.
 * Throws ConflictError when the row version no longer matches.
 */
